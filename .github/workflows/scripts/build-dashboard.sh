...
# Activates the Corepack-managed Yarn (Berry) pinned via packageManager, then installs.
# Bootstrapped through npm because Yarn cannot run a script before node_modules exists.
npm run install:ci
