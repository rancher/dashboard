...
function generateProvClusterObj(provClusterId, mgmtClusterId, state = 'active', kubernetesVersion = 'v1.27.10+rke2r1') {
  return {
...
function generateMgmtClusterObj(provClusterId, mgmtClusterId, state = 'active') {
  return {
    id:    mgmtClusterId,
...
function generateFakeNavClusterData(provClusterId = 'some-prov-cluster-id', mgmtClusterId = 'some-mgmt-cluster-id', addEditClusterCapabilities = false, state = 'active', kubernetesVersion = 'v1.27.10+rke2r1'): any {
  const fakeData = {
    provClusterObj:      generateProvClusterObj(provClusterId, mgmtClusterId, state, kubernetesVersion),
    mgmtClusterObj:      generateMgmtClusterObj(provClusterId, mgmtClusterId, state),
...
export function generateMockClusterDataAndIntercepts(
  clusters: { provId: string, mgmtId: string, state?: string, kubernetesVersion?: string }[] | string = 'some-prov-cluster-id',
  ...
  let clusterList: { provId: string, mgmtId: string, state?: string, kubernetesVersion?: string }[];

  if (typeof clusters === 'string') {
    clusterList = [{ provId: clusters, mgmtId: fakeMgmtClusterId || 'some-mgmt-cluster-id' }];
  } else {
    clusterList = clusters;
  }

  // Generate all fake cluster data
  const fakeClustersData = clusterList.map(({
    provId, mgmtId, state, kubernetesVersion
  }) => ({
    provId,
    mgmtId,
    data: generateFakeNavClusterData(provId, mgmtId, addEditClusterCapabilities, state, kubernetesVersion)
  }));

  // Intercept fleet clusters
  cy.intercept('GET', `/v1/fleet.cattle.io.clusters?*`, (req) => {
    req.continue((res) => {
      ...
      // Add all fake clusters
      fakeClustersData.forEach(({ data }) => {
        res.body.data.unshift(data.provClusterObj);
      });
      res.send(res.body);
    });
  }).as('fleetClusters');

  // Intercept provisioning clusters
  ...
  // Intercept management clusters
  cy.intercept('GET', `/v1/management.cattle.io.clusters?*`, (req) => {
    req.continue((res) => {
      fakeClustersData.forEach(({ data }) => {
        res.body.data.unshift(data.mgmtClusterObj);
      });
      res.send(res.body);
    });
  }).as('mgmtClusters');

  // Per-cluster endpoints (counts, namespaces, schemas, etc)
  fakeClustersData.forEach(({ mgmtId, data, provId }) => {
    cy.intercept('GET', `/k8s/clusters/${ mgmtId }/v1/counts?*`, (req) => {
      req.reply({
        statusCode: 200,
        body:       { data: data.fakeCountsReply },
      });
    }).as(`clusterCounts-${ mgmtId }`);

    cy.intercept('GET', `/k8s/clusters/${ mgmtId }/v1/namespaces?*`, (req) => {
      req.reply({
        statusCode: 200,
        body:       { data: data.fakeNamespacesReply },
      });
    }).as(`clusterNamespaces-${ mgmtId }`);

    cy.intercept('GET', `/k8s/clusters/${ mgmtId }/v1/schemas?*`, (req) => {
      req.reply({
        statusCode: 200,
        body:       {
          data: [
            data.fakeNodeSchema,
            data.fakeCountSchema,
            data.fakeNamespaceSchema,
            data.fakeDaemonsetSchema,
            data.fakePodSchema
          ]
        },
      });
    }).as(`clusterSchemas-${ mgmtId }`);

    cy.intercept('GET', `/k8s/clusters/${ mgmtId }/v1/pods?*`, (req) => {
      req.reply({
        statusCode: 200,
        body:       { data: [] },
      });
    }).as(`clusterPods-${ mgmtId }`);

    cy.intercept('GET', `/k8s/clusters/${ mgmtId }/v1/nodes?*`, (req) => {
      req.reply({
        statusCode: 200,
        body:       { data: [] },
      });
    }).as(`clusterNodes-${ mgmtId }`);

    cy.intercept('GET', `/k8s/clusters/${ mgmtId }/v1/apps.daemonsets?*`, (req) => {
      req.reply({
        statusCode: 200,
        body:       { data: data.fakeDaemonsetsReply },
      });
    }).as(`clusterDaemonsets-${ mgmtId }`);
  });

  // ... existing code for addEditClusterCapabilities (leave as is, only applies to first cluster)
  if (addEditClusterCapabilities && fakeClustersData.length > 0) {
    const { provId } = fakeClustersData[0];
    const data = fakeClustersData[0].data as any;

    // intercept node drivers for fake cluster edit scenario
    if (data.fakeNodeDriversReply) {
      cy.intercept('GET', `/v1/management.cattle.io.nodedrivers?*`, (req) => {
        req.reply({
          statusCode: 200,
          body:       { data: [data.fakeNodeDriversReply] },
        });
      }).as('nodeDrivers');
    }
    // intercept cloud credentials for fake cluster edit scenario
    if (data.fakeCloudCredentialsReply) {
      cy.intercept('GET', `/v3/cloudcredentials`, (req) => {
        req.reply({
          statusCode: 200,
          body:       { data: [data.fakeCloudCredentialsReply] },
        });
      }).as('cloudCreds');
    }
    // intercept machine config for fake cluster edit scenario
    if (data.fakeMachineConfigReply) {
      cy.intercept('GET', `/v1/rke-machine-config.cattle.io.digitaloceanconfigs/fleet-default/nc-${ provId }-pool1-${ MACHINE_POOL_ID }?*`, (req) => {
        req.reply({
          statusCode: 200,
          body:       data.fakeMachineConfigReply,
        });
      }).as('machineConfigDo');
    }
    // intercept individual cloud credential request
    if (data.fakeCloudCredIndividualReply) {
      cy.intercept('GET', `/v3/cloudcredentials/cattle-global-data:cc-${ CLOUD_CRED_ID }`, (req) => {
        req.reply({
          statusCode: 200,
          body:       data.fakeCloudCredIndividualReply,
        });
      }).as('individualCloudCred');
    }
    // intercept secrets req (for registries in cluster)
    if (data.fakeSecretsReply) {
      cy.intercept('GET', `/v1/secrets?*`, (req) => {
        req.reply({
          statusCode: 200,
          body:       { data: data.fakeSecretsReply },
        });
      }).as('secretsReply');
    }
    // ... existing code for Digital Ocean regions, sizes, images ...
  }

  return fakeClustersData;
}
