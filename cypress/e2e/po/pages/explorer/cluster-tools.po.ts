...
export default class ClusterToolsPagePo extends PagePo {
  ...
  featureChartCard(index: number): Cypress.Chainable {
    return this.featureChartCards().eq(index);
  }

  goToInstall(index: number) {
    return this.featureChartCard(index).find('.btn').contains('Install').click();
  }

  deleteChart(index: number) {
    return this.featureChartCard(index).find('.action .btn').eq(0).click();
  }

  editChart(index: number) {
    return this.featureChartCard(index).find('.action .btn').eq(1).click();
  }

  getChartVersion(index: number) {
    return this.featureChartCard(index).find('.version');
  }
}
