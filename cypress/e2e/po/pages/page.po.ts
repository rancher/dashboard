...
export default class PagePo extends ComponentPo {
  ...
  /**
   * Wait until a cluster is ready to be entered without tripping the fail-whale. Entering a cluster
   * runs the nav guard's loadCluster, which awaits unprotected schemas/counts/namespaces requests
   * against the downstream Steve proxy; if the proxy is momentarily reconnecting one rejects with a raw
   * "Network Error" and the app dead-ends on /fail-whale with no retry. So first confirm the mgmt
   * cluster is Connected/Ready (the same signal loadCluster checks), then poll those proxy endpoints
   * until they ALL return 200 across two consecutive rounds - a single 200 can be followed immediately
   * by a reconnect blip, so require the proxy to be consistently serving before navigating.
   */
