...
describe('Pod management and WebSocket interaction', { tags: ['@adminUser', '@standardUser', '@explorer'] }, () => {
  let token:string;
    ...
    cy.getRancherResource('v3', 'users?me=true').then((resp: Cypress.Response<any>) => {
      ...
      cy.createProject(projName, 'local', userId).then((resp: Cypress.Response<any>) => {
        ...
        cy.waitForRancherResource('v1', 'pods', podId, (resp) => {
          const podObject = resp.body.data.find((obj) => obj.id === `${ nsName }/${ podName }`);
  ...
  it('should create a new folder', () => {
    executeWebSocket('mkdir test-directory && echo "Directory created successfully"', podName, nsName, 'container-0', token).then((messages) => {
      expect(messages[2]).to.include('Directory created successfully');
    });
  });

  it('should validate the folder name', () => {
    executeWebSocket('ls', podName, nsName, 'container-0', token).then((messages) => {
      expect(messages[2]).to.include('test-directory');
    });
  });

  it('should delete the folder', () => {
    executeWebSocket('rm -rf test-directory && echo "Directory deleted successfully"', podName, nsName, 'container-0', token).then((messages) => {
