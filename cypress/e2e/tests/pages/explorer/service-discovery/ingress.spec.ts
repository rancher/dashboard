  ...
  describe('Create/Edit', { tags: ['@vai', '@adminUser'] }, () => {
    before('set up', () => {
      ...
      const createSecret = (index) => {
        const secretName = Cypress._.uniqueId(`e2e-${ Date.now().toString() }`);
        const body = {
        ...
        cy.createRancherResource('v1', 'secrets', body).then((resp) => {
          secretsNamesList.push(resp.body.metadata.name);
        });
      };

      // Create services
      const createService = (index) => {
        const serviceName = Cypress._.uniqueId(`e2e-${ Date.now().toString() }`);
        const body = {
          type:     'service',
          metadata: { namespace: 'default', name: serviceName },
          spec:     {
            ports: [{
              name: 'myport', port: 8080, protocol: 'TCP', targetPort: 80
            }],
        ...
        cy.createRancherResource('v1', 'services', body).then((resp) => {
          servicesNamesList.push(resp.body.metadata.name);
        });
      };

      // Chain the secret and services creation commands
