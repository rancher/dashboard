...
describe('CronJobs', { testIsolation: 'off', tags: ['@explorer2', '@adminUser'] }, () => {
  ...
  describe('List', { tags: ['@noVai', '@adminUser'] }, () => {
    ...
    // Fix: qa-tasks/1959
    it.only('Cronjob details page refresh dashboard', () => {
      ...
      jobDetailsPage.masthead().resourceStatus().should('be.visible', LONG_TIMEOUT_OPT).and(($el) => {
        const status = $el.text().trim();

        expect(['Running', 'Active', 'Pending', 'Creating', 'Succeeded']).to.include(status);
      });

      jobDetailsPage.masthead().resourceStatus().should('not.be.empty');
      ...
      cy.url().should('include', '/explorer/batch.cronjob/');

      const cronJobDetailsPage = new WorkloadsCronJobDetailPagePo(detailsPageCronJob, 'local', nsName3);

      cronJobDetailsPage.masthead().resourceStatus().should('be.visible', MEDIUM_TIMEOUT_OPT);

      // CronJob should NOT show stale "In Progress" status
      cronJobDetailsPage.masthead().resourceStatus().should('not.contain', 'In Progress');
