  ...
  describe('Workloads', () => {
    describe('Daemonsets', () => {
      it('modifying "Scaling and Upgrade Policy" to "On Delete" should use the correct property "OnDelete"', () => {
        ...
        workloadsDaemonsetsListPage.sharedComponents().baseResourceList().masthead().create();

        // create a new daemonset
        const workloadsDaemonsetsEditPage = new WorkLoadsDaemonsetsEditPagePo('local');

        workloadsDaemonsetsEditPage.sharedComponents().resourceDetail().createEditView().nameNsDescription()
          .name()
          .set(daemonsetName);
        workloadsDaemonsetsEditPage.containerImageInput().set('nginx');
        workloadsDaemonsetsEditPage.sharedComponents().resourceDetail().cruResource().saveOrCreate()
          .click();

        workloadsDaemonsetsListPage.waitForPage();
        workloadsDaemonsetsListPage.sharedComponents().baseResourceList().resourceTable().sortableTable()
          .rowElementWithName(daemonsetName)
          .should('be.visible');
        workloadsDaemonsetsListPage.sharedComponents().baseResourceList().actionMenu(daemonsetName).getMenuItem('Edit Config')
          .click();
