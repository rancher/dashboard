...
describe('Bundle Namespace Mappings', { testIsolation: 'off', tags: ['@fleet', '@adminUser'] }, () => {
  ...
  describe('CRUD', { tags: ['@fleet', '@adminUser'] }, () => {
    ...
    it('can create a bundle namespace mapping', () => {
      ...
      fleetBundleNsMappingsPage.baseResourceList().masthead().title().should('contain', 'BundleNamespaceMappings');
      headerPo.selectWorkspace(defaultWorkspace);
      fleetBundleNsMappingsPage.baseResourceList().masthead().createYaml();
      fleetBundleNsMappingsPage.createMappingForm().title().invoke('text').then((title) => {
      ...
      fleetBundleNsMappingsPage.resourceDetail().resourceYaml().codeMirror().value()
        .then((val) => {
        // convert yaml into json to update values
          ...
          fleetBundleNsMappingsPage.resourceDetail().resourceYaml().codeMirror().set(jsyaml.dump(json));
        });

      fleetBundleNsMappingsPage.resourceDetail().resourceYaml().saveOrCreate().click();
      ...
      fleetBundleNsMappingsPage.waitForPage();
      fleetBundleNsMappingsPage.mappingsList().rowWithName(customMappingName).checkVisible();
    });
    ...
    //   fleetBundleNsMappingsPage.goTo();
    //   fleetBundleNsMappingsPage.waitForPage();
    //   fleetBundleNsMappingsPage.mappingsList().actionMenu(customMappingName).getMenuItem('Edit YAML').click();
    ...
    it('can clone a bundle namespace mapping', () => {
      ...
      fleetBundleNsMappingsPage.goTo();
      fleetBundleNsMappingsPage.waitForPage();
      fleetBundleNsMappingsPage.mappingsList().actionMenu(customMappingName).getMenuItem('Clone').click();
      ...
      fleetBundleNsMappingsPage.waitForPage();
      fleetBundleNsMappingsPage.mappingsList().rowWithName(`${ customMappingName }-clone`).checkVisible();
    });
    ...
    it('can Download YAML', () => {
      ...
      fleetBundleNsMappingsPage.goTo();
      fleetBundleNsMappingsPage.waitForPage();
      fleetBundleNsMappingsPage.mappingsList().actionMenu(customMappingName).getMenuItem('Download YAML').click();
    ...
    it('can delete a bundle namespace mapping', () => {
      fleetBundleNsMappingsPage.goTo();
      fleetBundleNsMappingsPage.waitForPage();
      fleetBundleNsMappingsPage.mappingsList().actionMenu(`${ customMappingName }-clone`).getMenuItem('Delete').click();
      fleetBundleNsMappingsPage.mappingsList().resourceTable().sortableTable().rowNames('.col-link-detail')
        .then((rows: any) => {
          ...
          fleetBundleNsMappingsPage.waitForPage();
          fleetBundleNsMappingsPage.mappingsList().resourceTable().sortableTable().checkRowCount(false, rows.length - 1);
          fleetBundleNsMappingsPage.mappingsList().resourceTable().sortableTable().rowNames('.col-link-detail')
