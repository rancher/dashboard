...
describe('Cluster Registration Tokens', { testIsolation: 'off', tags: ['@fleet', '@adminUser'] }, () => {
  ...
  describe('CRUD', { tags: ['@fleet', '@adminUser'] }, () => {
    ...
    it('can create a cluster registration token', () => {
      ...
      fleetTokensPage.baseResourceList().masthead().title().should('contain', 'Cluster Registration Tokens');
      headerPo.selectWorkspace(defaultWorkspace);
      fleetTokensPage.baseResourceList().masthead().createYaml();
      fleetTokensPage.createTokenForm().waitForPage('as=yaml');
      fleetTokensPage.createTokenForm().title().invoke('text').then((title) => {
        expect(title.replace(/\s+/g, ' ')).to.contain('Cluster Registration Token: Create');
      });
      fleetTokensPage.resourceDetail().resourceYaml().codeMirror().value()
        .then((val) => {
        // convert yaml into json to update values
          ...
          fleetTokensPage.resourceDetail().resourceYaml().codeMirror().set(jsyaml.dump(json));
        });

      fleetTokensPage.resourceDetail().resourceYaml().saveOrCreate().click();
      ...
      fleetTokensPage.waitForPage();
      fleetTokensPage.tokensList().rowWithName(customTokenName).checkVisible();
    });
    ...
    it('can Edit Config', () => {
      ...
      fleetTokensPage.goTo();
      fleetTokensPage.waitForPage();
      fleetTokensPage.tokensList().actionMenu(customTokenName).getMenuItem('Edit YAML').click();
    ...
    it('can clone a cluster registration token', () => {
      ...
      fleetTokensPage.goTo();
      fleetTokensPage.waitForPage();
      fleetTokensPage.tokensList().actionMenu(customTokenName).getMenuItem('Clone').click();
      ...
      fleetTokensPage.waitForPage();
      fleetTokensPage.tokensList().rowWithName(`${ customTokenName }-clone`).checkVisible();
    });
    ...
    it('can Download YAML', () => {
      ...
      fleetTokensPage.goTo();
      fleetTokensPage.waitForPage();
      fleetTokensPage.tokensList().actionMenu(customTokenName).getMenuItem('Download YAML').click();
    ...
    it('can delete a cluster registration token', () => {
      fleetTokensPage.goTo();
      fleetTokensPage.waitForPage();
      fleetTokensPage.tokensList().actionMenu(`${ customTokenName }-clone`).getMenuItem('Delete').click();
      fleetTokensPage.tokensList().resourceTable().sortableTable().rowNames('.col-link-detail')
        .then((rows: any) => {
          ...
          cy.wait('@deleteToken');
          fleetTokensPage.waitForPage();
          fleetTokensPage.tokensList().resourceTable().sortableTable().checkRowCount(false, rows.length - 1);
          fleetTokensPage.tokensList().resourceTable().sortableTable().rowNames('.col-link-detail')
