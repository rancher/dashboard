...
describe('GitRepo Restrictions', { testIsolation: 'off', tags: ['@fleet', '@adminUser'] }, () => {
  ...
  describe('CRUD', { tags: ['@fleet', '@adminUser'] }, () => {
    ...
    it('can create a gitrepo restriction', () => {
      ...
      fleetRestrictionsPage.baseResourceList().masthead().title().should('contain', 'GitRepoRestrictions');
      headerPo.selectWorkspace(defaultWorkspace);
      fleetRestrictionsPage.baseResourceList().masthead().createYaml();
      fleetRestrictionsPage.createRestrictionForm().waitForPage('as=yaml');
      fleetRestrictionsPage.createRestrictionForm().title().invoke('text').then((title) => {
        expect(title.replace(/\s+/g, ' ')).to.contain('GitRepoRestriction: Create');
      });
      fleetRestrictionsPage.resourceDetail().resourceYaml().codeMirror().value()
        .then((val) => {
        // convert yaml into json to update values
          ...
          fleetRestrictionsPage.resourceDetail().resourceYaml().codeMirror().set(jsyaml.dump(json));
        });

      fleetRestrictionsPage.resourceDetail().resourceYaml().saveOrCreate().click();
      ...
      fleetRestrictionsPage.waitForPage();
      fleetRestrictionsPage.restrictionList().rowWithName(customRestrictionName).checkVisible();
    });
    ...
    //   fleetRestrictionsPage.goTo();
    //   fleetRestrictionsPage.waitForPage();
    //   fleetRestrictionsPage.restrictionList().actionMenu(customRestrictionName).getMenuItem('Edit YAML').click();
    ...
    it('can clone a gitrepo restriction', () => {
      ...
      fleetRestrictionsPage.goTo();
      fleetRestrictionsPage.waitForPage();
      fleetRestrictionsPage.restrictionList().actionMenu(customRestrictionName).getMenuItem('Clone').click();
      ...
      fleetRestrictionsPage.waitForPage();
      fleetRestrictionsPage.restrictionList().rowWithName(`${ customRestrictionName }-clone`).checkVisible();
    });
    ...
    it('can Download YAML', () => {
      ...
      fleetRestrictionsPage.goTo();
      fleetRestrictionsPage.waitForPage();
      fleetRestrictionsPage.restrictionList().actionMenu(customRestrictionName).getMenuItem('Download YAML').click();
    ...
    it('can delete a gitrepo restriction', () => {
      fleetRestrictionsPage.goTo();
      fleetRestrictionsPage.waitForPage();
      fleetRestrictionsPage.restrictionList().actionMenu(`${ customRestrictionName }-clone`).getMenuItem('Delete').click();
      fleetRestrictionsPage.restrictionList().resourceTable().sortableTable().rowNames('.col-link-detail')
        .then((rows: any) => {
          ...
          fleetRestrictionsPage.waitForPage();
          fleetRestrictionsPage.restrictionList().resourceTable().sortableTable().checkRowCount(false, rows.length - 1);
          fleetRestrictionsPage.restrictionList().resourceTable().sortableTable().rowNames('.col-link-detail')
