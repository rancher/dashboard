import { FleetClusterPagePo } from '@/cypress/e2e/po/pages/fleet/fleet.cattle.io.cluster.po';
...
describe('Fleet Clusters', { tags: ['@fleet', '@adminUser'] }, () => {
  const fleetClusterPage = new FleetClusterPagePo();
  const fleetGitRepoPage = new FleetGitRepoPagePo();
  ...
  describe('bundle manifests are deployed from the BundleDeployment into the downstream cluster', { testIsolation: 'off', tags: ['@fleet', '@adminUser', '@jenkins'] }, () => {
    ...
    it('data is populated in fleet cluster list and detail view', () => {
      ...
      fleetGitRepoPage.gitRepoList().resourceTableDetails(gitRepo, 1).contains('Active', LONG_TIMEOUT_OPT);

      // go to fleet clusters
      FleetClusterPagePo.navTo();
      fleetClusterPage.waitForPage();
      headerPo.selectWorkspace(namespace);
      fleetClusterPage.clusterList().list().resourceTable().sortableTable()
        .checkLoadingIndicatorNotVisible();

      // check name
      fleetClusterPage.clusterList().resourceTableDetails(clusterName, 2).should('be.visible');
      // check cluster state in fleet
      fleetClusterPage.clusterList().resourceTableDetails(clusterName, 1).contains('Not Ready', MEDIUM_TIMEOUT_OPT);
      fleetClusterPage.clusterList().resourceTableDetails(clusterName, 1).contains('Active', LONG_TIMEOUT_OPT);
      // check bundles ready
      fleetClusterPage.clusterList().resourceTableDetails(clusterName, 3).should('have.text', '4');
      // check repos ready
      fleetClusterPage.clusterList().resourceTableDetails(clusterName, 4).should('have.text', '1');
      // check resources: testing https://github.com/rancher/dashboard/issues/11154
      fleetClusterPage.clusterList().resourceTableDetails(clusterName, 5).contains( ' 15 ', MEDIUM_TIMEOUT_OPT);
      ...
      fleetClusterPage.clusterList().goToDetailsPage(clusterName);
    ...
    it('can Pause', () => {
      ...
      // pause
      fleetClusterPage.clusterList().list().actionMenu(clusterName).getMenuItem('Pause')
        .click();
      ...
      // check cluster state
      fleetClusterPage.clusterList().resourceTableDetails(clusterName, 1).contains('Paused', LONG_TIMEOUT_OPT);
    });
    ...
    it('can Unpause', () => {
      ...
      // unpause
      fleetClusterPage.clusterList().list().actionMenu(clusterName).getMenuItem('Unpause')
        .click();
    ...
    it('can Edit Config', () => {
      ...
      fleetClusterPage.waitForPage();
      headerPo.selectWorkspace(namespace);
      fleetClusterPage.clusterList().list().actionMenu(clusterName).getMenuItem('Edit Config')
        .click();

      const editFleetCluster = fleetClusterPage.editFleetCluster(undefined, clusterName);
    ...
    it('can Download YAML', () => {
      ...
      fleetClusterPage.waitForPage();
      headerPo.selectWorkspace(namespace);
      fleetClusterPage.clusterList().list().actionMenu(clusterName).getMenuItem('Download YAML')
        .click();
    ...
    it('can assign cluster to different fleet workspaces', () => {
      ...
      // go to fleet clusters
      fleetClusterPage.goTo();
      fleetClusterPage.waitForPage();
      headerPo.selectWorkspace(namespace);

      cy.intercept('PUT', '/v1/userpreferences/*').as('changeWorkspace');
      fleetClusterPage.clusterList().list().actionMenu(clusterName).getMenuItem('Change workspace')
        .click();
      fleetClusterPage.changeWorkspaceForm().workspaceSelect().toggle();
      fleetClusterPage.changeWorkspaceForm().workspaceSelect().clickOptionWithLabel(customWorkspace);
      fleetClusterPage.changeWorkspaceForm().applyAndWait('v3/clusters/*');
      fleetClusterPage.clusterList().list().resourceTable().sortableTable()
      ...
      fleetClusterPage.clusterList().resourceTableDetails(clusterName, 2).isVisible();

      // restore
      fleetClusterPage.clusterList().list().actionMenu(clusterName).getMenuItem('Change workspace')
        .click();
      fleetClusterPage.changeWorkspaceForm().workspaceSelect().toggle();
      fleetClusterPage.changeWorkspaceForm().workspaceSelect().clickOptionWithLabel(namespace);
      fleetClusterPage.changeWorkspaceForm().applyAndWait('v3/clusters/*');
      fleetClusterPage.clusterList().list().resourceTable().sortableTable()
        .checkRowCount(true, 1, MEDIUM_TIMEOUT_OPT);
  ...
  describe('List', { tags: ['@fleet', '@adminUser'] }, () => {
    ...
    it('Git Repos Tab Add Repository button takes you to the correct page', () => {
      ...
      fleetClusterPage.clusterList().list().resourceTable().sortableTable()
        .checkLoadingIndicatorNotVisible();
      fleetClusterPage.clusterList().goToDetailsPage('local');
      fleetClusterDetailsPage.waitForPage(null, 'repos');
    ...
    it('should only display action menu with allowed actions only', () => {
      ...
      const constActionMenu = fleetClusterPage.clusterList().list().resourceTable().sortableTable()
    ...
    it('check table headers are available in list and details view', { tags: ['@vai', '@adminUser'] }, () => {
      ...
      fleetGitRepoPage.waitForPage();
      headerPo.selectWorkspace(workspace);
      fleetGitRepoPage.gitRepoList().list().rowWithName(gitRepo).checkVisible();
      ...
      fleetClusterPage.clusterList().resourceTableDetails(clusterName, 2).find('a').click();
