...
describe('Fleet HelmOps', { testIsolation: 'off', tags: ['@fleet', '@adminUser'] }, () => {
  ...
  describe('Create and Edit HelmOp with Secrets and ConfigMaps', () => {
    ...
    it('Can create a HelmOp with Secrets and ConfigMaps', () => {
      ...
      // Chart step
      helmOpCreatePage.setChart('jenkins');
      helmOpCreatePage.setRepository('https://charts.jenkins.io');
      helmOpCreatePage.setVersion('5.8.61');
      helmOpCreatePage.resourceDetail().createEditView().nextPage();
    ...
    it('Can edit a HelmOp to remove and add Secrets and ConfigMaps', () => {
      ...
      // Wait for update request (handles 409 conflicts with automatic retries)
      cy.wait('@updateHelmOp').then((interception) => {
        // If 409 conflict, wait for retry
        if (interception.response?.statusCode === 409) {
          cy.log('HelmOp update failed with 409 error, waiting for retry...');
