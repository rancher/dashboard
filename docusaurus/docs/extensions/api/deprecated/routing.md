...
## Routes definition for an Extension as a top-level product (deprecated)
If you are not familiar with the concept of "an Extension as a top-level product", we recommend that your about the concept **[here](./concepts-deprecated.md#what-is-a-top-level-product)**.

> Legacy DSL doc. The recommended routing flow is summarized on the main Products page. You can still add bespoke routes with `plugin.addRoutes`; they coexist with the routes generated by `addProduct`/`extendProduct` configs.
...
### Top-level product - entry routes for each page definition (deprecated)
...
### Top-level product - adding your defined routes to Vue Router (deprecated)
...
Just to reinforce the message, it is imperative that the `name` and `path` follow this convention needed for Extension top-level products, which we cover on this [overview](./concepts-deprecated.md#overview-on-routing-structure-for-a-top-level-extension-product).
...
## Routes definitions for an Extension as a cluster-level product (deprecated)
If you are not familiar with the concept of "an Extension as a cluster-level product", we recommend that your about the concept **[here](./concepts-deprecated.md#what-is-a-cluster-level-product)**.
...
### Cluster-level product - entry routes for each page definition (deprecated)
...
### Cluster-level product - adding your defined routes to Vue Router (deprecated)
