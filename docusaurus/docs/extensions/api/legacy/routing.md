...
If you are not familiar with the concept of "an Extension as a top-level product", we recommend that your about the concept **[here](./concepts-legacy.md#what-is-a-top-level-product)**.
...
Just to reinforce the message, it is imperative that the `name` and `path` follow this convention needed for Extension top-level products, which we cover on this [overview](./concepts-legacy.md#overview-on-routing-structure-for-a-top-level-extension-product).
...
If you are not familiar with the concept of "an Extension as a cluster-level product", we recommend that your about the concept **[here](./concepts-legacy.md#what-is-a-cluster-level-product)**.
