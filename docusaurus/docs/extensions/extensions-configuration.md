...
To become familiar with routing on VueJS and route definition we recommend that you should give a read about the [https://v3.router.vuejs.org/guide/](essentials on Vue Router) and also the definition of a [Vue Router route](https://v3.router.vuejs.org/api/#routes).
...
This ensures that all the wiring needed for the Extension to work properly on Rancher Dashboard is done. There's even the case where the wildcard `resource` needs to be defined in order to display information about Kubernetes resources or custom CRDs. An example of a resource route in a top-level Extension product would be:
...
With the route definition in the router (check the [#extension-routing](Extension Routing) chapter), you can define which Vue component will be loaded as a custom page. That will act as a "blank canvas" to render anything you want.
...
|`showState`| Boolean | If the 'age' column is available on the list view |
...
> Note: On the example the entry point for your Extension product will be the page named `CUSTOM_PAGE_NAME`, as it's defined on the `to` parameter in the `product` options.
...
In the example provided above we are registering 3 pages: 1 is a "resource" page (`YOUR_K8S_RESOURCE_NAME`) and 2 are "custom" pages (`CUSTOM_PAGE_NAME_1` and `CUSTOM_PAGE_NAME_2`). Those are set as side-menu entries being `YOUR_K8S_RESOURCE_NAME` and `CUSTOM_PAGE_NAME_1` in a group called `advanced` and another page (`CUSTOM_PAGE_NAME_2`) as a root level side-menu entry. 

The default ordering of these side-menu entries is the order on which you register them using `basicType`. In the above example the side-menu output would be something like:
...
- We are setting two root level side-menu items: `CUSTOM_PAGE_NAME_2` and `advanced`
- Since we are giving a weight of 2 to `advanced` (the bigger, the higher it will sit on the menu ordering) and a weight of 1 to `CUSTOM_PAGE_NAME_2`, the `advanced` menu will be above the `CUSTOM_PAGE_NAME_2` side-menu entry
- Inside the `advanced` group we are setting a specific order as well: weight of 2 to `CUSTOM_PAGE_NAME_1` and a weight of 1 to `YOUR_K8S_RESOURCE_NAME`
- This will make the side-menu entry for `CUSTOM_PAGE_NAME_1` appear higher than `YOUR_K8S_RESOURCE_NAME` inside the group `advanced`
...
## Extension Routing definition - TO BE REVIEWED!!!!!
...
const routes = [
  ...
  {
    name:      `${ YOUR_PRODUCT_NAME }-c-cluster-myCustomPage`,
    path:      `/${ YOUR_PRODUCT_NAME }/c/:cluster/myCustomPage`,
...
On the above example, we are setting a "base" route for our **top-level product** that the Extension is registering. It is imperative that the `name` and `path` follow this convention, because this will tie-in to the whole loading logic of Rancher Dashboard. So `name` and `path` should be as of the following type:
```ts
{
...
When defining routes for a top-level product from an Extension, all the remaining routes registered should be children of you base route in terms of `name` and `path`. This is very important as your routes might not work properly!. 

On a more advanced top-level product definition which come from an Extension, a user might want to list/create/edit a kubernetes resource providing an interface for it. One can leverage the usage of the default interfaces for a list/create/edit views used on Rancher Dashboard, such as:
...
const routes = [
  {
    name:      `${ YOUR_PRODUCT_NAME }-c-cluster-dashboard`,
