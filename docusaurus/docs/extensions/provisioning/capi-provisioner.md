...
- **Infrastructure cluster**: the provider's cluster-scoped CRD (e.g. `AWSCluster`), referenced from `provisioning.cattle.io.cluster.spec.rkeConfig.infrastructureRef`. This resource has no equivalent in the classic node-driver flow — the extension is responsible for creating and managing it directly.
