...
## Spoofed type (synthetic resource)

A spoofed type lets you display data from external APIs or computed data as if it were a Kubernetes resource. You provide a `getInstances` function that returns the data, and Rancher Dashboard renders it using the standard list/detail views:
...
export default function(plugin: IPlugin) {
  ...
  plugin.addProduct(product, [backupsPage]);
}
```

**Result:** A product with a "Backups" page that displays the two backup entries in a standard Rancher resource list. A schema is auto-generated for the spoofed type.

---

## Real-world: mixed product with groups, resource pages, and spoofed types

A complete product combining custom pages in a group, a resource page with DSL options, and a spoofed type:
...
export default function(plugin: IPlugin) {
  ...
  // Custom pages in a group with its own overview page
  const alertsPage: ProductChildCustomPage = {
    name:      'alerts',
    label:     'Alerts',
    component: () => import('./pages/Alerts.vue'),
  ...
  // Resource page with DSL options
  const clusterPage: ProductChildResourcePage = {
    type:                     'provisioning.cattle.io.cluster',
  ...
  // Spoofed type (synthetic resource from external data)
  const backupsPage: ProductChildSpoofedTypePage = {
    name:         'backups',
    label:        'Backups',
    getInstances: () => fetch('/api/backups').then((r) => r.json()),
  };
  ...
  plugin.addProduct(product, [monitoringGroup, clusterPage, backupsPage]);
}
```
...
```
▾ Monitoring          (group with overview page)
    Alerts            (custom page)
    Metrics           (custom page)
Provisioned Clusters  (resource page, renamed, no bulk actions)
Backups               (spoofed type)
```
