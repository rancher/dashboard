...
export default {
  ...
  props: {
    ...
    createPath: {
      type:    Object,
      default: () => ({ name: 'rbac-resource-create', params: { resource: RBAC.CLUSTER_ROLES } })
    }
  },
  ...
  computed:   {
     ...
     */
    clusterSchema() {
      return this.$store.getters['cluster/schemaFor'](RBAC.CLUSTER_ROLES);
    },
    /**
     * Returns RBAC Cluster Role Headers
     */
    headers() {
      return FRIENDLY[RBAC.CLUSTER_ROLES].headers;
    },
  },
};
</script>
<template>
  <div>
    ...
    <TabbedLinks :tab-list="tabList">
      <ResourceTable :rows="clusterRows" :schema="clusterSchema" :headers="headers" />
    </TabbedLinks>
  </div>
</template>
