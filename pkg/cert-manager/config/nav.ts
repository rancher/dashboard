 ...
 *
 * The product registration API has no way to set this on a page - `configurePageItem` only sets it
 * for a group that has its own component, and moving the overview onto the group would lose the
 * `ifHaveType` gate that hides the whole group when cert-manager is not installed. The store's
