...
export default {
  components: { Drawer, ExplainPanel },

  fetch() {
    const typeName = this.$route.params?.resource;
    const schema = typeName ? this.$store.getters[`cluster/schemaFor`](typeName) : undefined;
    const cluster = this.$store.getters['currentCluster'] || 'local';

    openAPI.get(cluster?.id, this.$store.dispatch).then((data) => {
      this.load(data, schema);
    }).catch((e) => {
      this.load(undefined, schema, e);
    });
  },
        ...
        >
          ...
          >
            <span
              ...
              class="ml-5 mr-5"
            >&gt;</span>
            <span v-if="i === breadcrumbs.length - 1">{{ b.name }}
            </span>
            <a
              v-else
              href="#"
