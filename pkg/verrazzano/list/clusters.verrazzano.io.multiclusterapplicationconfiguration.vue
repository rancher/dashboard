...
import { VZ_MC_APPLICATION } from '@pkg/types';

import { allHash } from '@shell/utils/promise';

export default {
  name:       'ListMultiClusterApplicationConfiguration',
  ...
  async fetch() {
    ...
    if (isVerrazzano) {
      requests = { allApplications: this.$store.dispatch('management/findAll', { type: VZ_MC_APPLICATION }) };
    } else {
      requests = { allApplications: this.$store.dispatch('cluster/findAll', { type: VZ_MC_APPLICATION }) };
    }
