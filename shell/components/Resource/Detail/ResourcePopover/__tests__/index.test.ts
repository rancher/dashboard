...
describe('component: ResourcePopover/index.vue', () => {
  ...
  const PopoverCardStub = {
    PopoverCard: {
      // Stub PopoverCard to easily test slots and props
      template: `
              <div>
                <slot />
  ...
  describe('data Fetching and Rendering', () => {
    it('should display a loading indicator while fetching data', async() => {
      mockClusterFind.mockImplementation(() => new Promise(() => {})); // Pending promise
  ...
  describe('computed Properties', () => {
    it('resourceTypeLabel: should be empty if data is not loaded', async() => {
      ...
      const parsedLabel = JSON.parse(ariaLabel || '{}');

      expect(parsedLabel.args.resource).toBe('');
    });

    it('resourceTypeLabel: should use parentNameOverride when available', async() => {
      mockClusterFind.mockResolvedValue(mockResource); // mockResource has parentNameOverride
      const wrapper = createWrapper();
      ...
      expect(parsedLabel.args.resource).toBe(mockResource.parentNameOverride);
    });
    ...
    it('resourceTypeLabel: should use type-map label as a fallback', async() => {
      ...
      expect(parsedLabel.args.resource).toBe('Pod'); // From the store mock
    });
  });
  ...
  describe('props passed to child components', () => {
    it('should pass correct props to PopoverCard', async() => {
      ...
      const expectedAriaLabel = JSON.stringify({
        key:  'component.resource.detail.glance.ariaLabel.showDetails',
        args: { name: mockResource.nameDisplay, resource: mockResource.parentNameOverride }
      });
    ...
    it('should pass correct props to ActionMenu', async() => {
      ...
      const expectedAriaLabel = JSON.stringify({
        key:  'component.resource.detail.glance.ariaLabel.actionMenu',
        args: { resource: mockResource.nameDisplay }
      });
