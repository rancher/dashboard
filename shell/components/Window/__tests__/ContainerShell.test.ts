...
/**
 * `ContainerShell.vue` is a plain-JS SFC, so `vue-tsc` cannot infer its `methods` block.
 * Narrow interface for jest.spyOn on methods.
 * Remove once the component is converted to `<script lang="ts"> defineComponent`.
 */
interface ContainerShellMethods {
  connect: () => void;
}
...
describe('component: ContainerShell', () => {
  ...
  const resetMocks = () => {
    ...
    defaultContainerShellParams.props.pod.os = 'linux';
  ...
  it('the socket is instantiated', async() => {
    ...
    const socketParams = jest.mocked(Socket).mock.calls[0][0]
      .split('?')[1]
      .split('&')
      .reduce((paramMap: Record<string, string>, param: string) => {
    ...
    expect(jest.mocked(Socket).mock.calls).toHaveLength(1);
  ...
  it('the sockets events are bound', async() => {
    ...
    const addEventListenerCalls = jest.mocked(addEventListener).mock.calls;
  ...
  it('the socket disconnect event with an error sets data props correctly and attempts a second connect', async() => {
    resetMocks();
    const consoleError = jest.spyOn(console, 'error').mockImplementation(() => {});
    const connect = jest.spyOn((ContainerShell as unknown as { methods: ContainerShellMethods }).methods, 'connect');
    const wrapper = await wrapperPostMounted(defaultContainerShellParams);
    const errorMessage = 'eventMessageError';
    ...
    expect(defaultContainerShellParams.props.pod.os).toBeUndefined();
  ...
  it('the socket disconnect event fires twice, sets data props correctly, and only attempts two connects if the pod os is undefined', async() => {
    ...
    const testUndefinedOsParams = {
      ...defaultContainerShellParams,
      props: {
        ...defaultContainerShellParams.props,
        pod: {
          ...defaultContainerShellParams.props.pod,
          os: undefined
        }
      }
    };
    ...
    expect(wrapper.vm.backupShells).toHaveLength(1);
    expect(wrapper.vm.os).toBe('linux');
    expect(testUndefinedOsParams.props.pod.os).toBeUndefined();
  ...
  it('the socket disconnect event fires 3 times, sets data props correctly, and only attempts 3 connects if the pod os is defined at the pods parent node', async() => {
    ...
    expect(wrapper.vm.backupShells).toHaveLength(1);
    expect(wrapper.vm.os).toBe('linux');
    expect(testNodeDefinedOsParams.props.pod.os).toBe('linux');
    expect(wrapper.vm.errorMsg).toContain(linuxErrorMessage);
