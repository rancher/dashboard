...
export default Vue.extend<Data, any, any, any>({
  ...
  methods: {
    ...
    async loadSourceCache() {
      this.selectedAccOrOrg = this.value.selectedAccOrOrg;
      if (this.selectedAccOrOrg) {
        await this.fetchRepos()
          .then(() => {
            if (this.repos.length && !this.hasError.repo) {
              this.selectedRepo = this.value.selectedRepo;
          ...
          .then(() => {
            if (this.branches.length && !this.hasError.branch) {
              this.selectedBranch = this.value.selectedBranch;
        ...
        const commit = this.commits.find((c: commit) => {
          ...
          return sha === this.value.selectedCommit.sha;
        });
