...
export default {
  ...
  methods: {
    ...
    getOptionLabel(option) {
      ...
      if (typeof this._options?.[0] === 'object') {
    ...
    getUpdatedOption(option) {
      const isOutdated = this._options && !this._options.find((opt) => option[this.optionLabel] === opt[this.optionLabel]);

      return isOutdated ? this._options.find((opt) => isEqual(this.reduce(option), this.reduce(opt))) : undefined;
    },
