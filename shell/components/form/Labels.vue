...
export default {
  ...
  computed: {
    containerClass(): any {
      return `${ (this as any).displaySideBySide ? 'row' : '' } ${ (this as any).defaultContainerClass }`.trim();
    },

    sectionClass(): any {
      return `${ (this as any).displaySideBySide ? 'col span-6' : 'row' } ${ (this as any).defaultSectionClass }`.trim();
    },

    columnsClass(): any {
      return `${ (this as any).displaySideBySide ? 'col span-6' : 'row' }`.trim();
    },

    showToggler(): any {
      return (this as any).mode === _VIEW && ((this as any).labels.hasProtectedKeys || (this as any).annotations.hasProtectedKeys);
    }
  }
};
</script>
  ...
  <div :class="containerClass">
          ...
          <slot name="labels">
            <template>
              <KeyValue
                ...
                @input="labels.update($event, (x) => value.setLabels(x))"
              />
            </template>
          </slot>
        </div>
    ...
    >
      <KeyValue
        ...
        @input="annotations.update($event, (x) => value.setAnnotations(x))"
      />
    </div>
  </div>
</template>
