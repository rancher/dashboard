...
/**
 * A copy of KeyValue laid out for use inside RcSection: the title, rows and
 * footer are stacked with RcContentGroup, so rows sit 16px apart instead of
 * relying on per-item margins.
 */
export default {
  name: 'RcKeyValue',
  ...
  computed: {
    ...
    rowStyle() {
      ...
      // Each row is its own grid, so columns must not size to their content or they would not line up between rows
      return `grid-template-columns: repeat(${ size }, minmax(0, 1fr))${ gap };`;
    },
    usedKeyOptions() {
...
<template>
  <RcContentGroup class="rc-key-value">
    <div
    ...
    <!-- Only render the grid when it has content, so an empty grid doesn't add a gap above the footer -->
    <div
      v-if="rows.length || isView"
      class="kv-container"
      role="grid"
    ...
    >
      ...
      <RcContentGroup v-if="!rows.length && isView">
        <div
          class="rowgroup"
          role="rowgroup"
        >
      ...
      <RcContentGroup v-else-if="rows.length">
        <div
          v-for="(row,i) in rows"
          :key="i"
          class="rowgroup"
...
.rc-key-value {
  ...
  // The header labels sit closer to the first row than the rows sit to each other
  .rowgroup.header {
    margin-bottom: 8px;
  }

  // Every row is its own grid; rowStyle gives them all the same columns
  .row {
    &::before {
      display: none;
    }
