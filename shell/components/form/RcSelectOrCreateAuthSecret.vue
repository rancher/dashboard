...
<template>
  <RcContentGroup>
    ...
    >
      <div
        :class="vertical ? 'vertical-stack' : 'row'"
      >
        <div :class="firstCol">
          <LabeledSelect
...
.select-or-create-auth-secret {
  ...
  // In vertical mode every field sits on its own line, so stack them with the same spacing as RcContentGroup
  .vertical-stack {
    display: flex;
    flex-direction: column;
    gap: var(--gap-md, 16px);
