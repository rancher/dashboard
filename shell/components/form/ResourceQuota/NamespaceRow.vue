...
export default {
  ...
  mounted() {
    // View mode renders the live store model rather than a clone, so writing back here would silently replace
    // the namespace's real (possibly malformed) annotation in the store with the project defaults.
    if (this.mode === _VIEW) {
      return;
    }
