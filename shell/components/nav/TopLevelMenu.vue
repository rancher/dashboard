...
import { MENU_MAX_CLUSTERS, PINNED_CLUSTERS, RECENT_CLUSTERS } from '@shell/store/prefs';
...
export default {
  ...
  computed: {
    ...
    showClusterSearch() {
      return this.allClustersCount > MENU_MAX_CLUSTERS;
    },
    ...
    // ALL CLUSTERS lists the COMPLETE estate — including the pinned and recent clusters (they still show
    // in their own groups above; the pin state is shown in place here). Alphabetical, local excluded
    // (it has its own fixed slot). While searching this is the flat match list. SURE-8192 rev 2.
    railAll() {
      ...
      [...this.pinFiltered, ...this.recentClusters, ...this.clustersFiltered]
        .filter((c) => !c.isLocal)
        .forEach((c) => byId.set(c.id, c));

      return sortBy([...byId.values()], 'label');
    },
  ...
  watch: {
    ...
    // on its own whenever a pref changes — the optimistic commit AND setMerge's reconcile commit each flow
...
<style lang="scss" scoped>
  ...
  // v2: the shelf already conveys pinned-ness (the PINNED group + the pin toggle), so ClusterIconMenu's
  // pin overlay on the chip is redundant here — hide it in the switcher. SURE-8192 (v2).
  .side-menu :deep(.cluster-pin-icon) {
    display: none;
  }
    ...
    .body {
      ...
      .clusters {
        ...
        &::after {
          ...
          margin-top: -8px;
          pointer-events: none;
          background: linear-gradient(180deg, rgba(18, 20, 26, 0) 0%, rgba(18, 20, 26, 0.08) 100%);
