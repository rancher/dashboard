...
import { Layout, Position, Tab } from '../index.vue';
...
export default (props?: { positions: Position[], layout: Layout }) => {
  ...
  const isPanelEnabled = computed(() => (props?.positions || []).reduce((acc, pos) => ({
    ...acc,
    [pos]: store.state.wm.open[pos] && !store.state.wm.tabs.find((t: Tab) => !t.layouts.includes(props?.layout || 'default') && t.position === pos),
