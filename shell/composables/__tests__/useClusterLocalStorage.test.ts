...
describe('composable: useClusterLocalStorage', () => {
  ...
  describe('pruning stale clusters (local, pagination off)', () => {
    it('should remove entries for clusters that no longer exist', () => {
      localStorage.setItem(navKey('c-gone'), JSON.stringify({ a: true }));
      localStorage.setItem(navKey('c-abc'), JSON.stringify({ b: true }));
      mockGetters['management/all'] = () => [{ id: 'c-abc' }];

      useClusterLocalStorage('nav-group-state', () => 'c-abc');
    ...
    it('should not prune again on save, which happens on every navigation', () => {
      mockGetters['management/all'] = () => [{ id: 'c-abc' }];
    ...
    it('should prune again once the cluster changes', async() => {
      const clusterId = ref('c-abc');

      mockGetters['management/all'] = () => [{ id: 'c-two' }];
      useClusterLocalStorage('nav-group-state', clusterId);
  ...
  describe('pruning stale clusters (paginated)', () => {
    ...
    it('should not query the API on save, which happens on every navigation', async() => {
