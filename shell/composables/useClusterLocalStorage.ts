 ...
 *
 * Entries belonging to clusters that no longer exist are pruned once per
 * cluster, so removed clusters don't leave their data behind. That check can hit
 * the API, so it deliberately isn't tied to saving, which happens on every
 * navigation.
 *
...
export function useClusterLocalStorage<T>(
  ...
  /**
   * Serializes and stores `value` for the current cluster. No-op when there is
   * no cluster or `localStorage` is unavailable.
   */
  ...
  // Prune once for each cluster we start using, rather than on every write.
  watch(storageKey, (newStorageKey) => {
    if (newStorageKey) {
      pruneStaleClusters();
    }
  }, { immediate: true });
