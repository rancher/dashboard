...
describe('Settings Configuration', () => {
  describe('DELETE_INFRA_MACHINE_ON_FAILURE_AFTER', () => {
    it('should be defined in SETTING constants', () => {
      expect(SETTING.DELETE_INFRA_MACHINE_ON_FAILURE_AFTER).toBe('delete-infra-machine-on-failure-after');
    });

    it('should be included in ALLOWED_SETTINGS', () => {
      expect(ALLOWED_SETTINGS).toHaveProperty(SETTING.DELETE_INFRA_MACHINE_ON_FAILURE_AFTER);
    });

    it('should be included in PROVISIONING_SETTINGS', () => {
      expect(PROVISIONING_SETTINGS).toContain(SETTING.DELETE_INFRA_MACHINE_ON_FAILURE_AFTER);
    });

    it('should have the correct configuration in ALLOWED_SETTINGS', () => {
      const config = ALLOWED_SETTINGS[SETTING.DELETE_INFRA_MACHINE_ON_FAILURE_AFTER];
  ...
  describe('PROVISIONING_SETTINGS array', () => {
    ...
    it('should maintain proper order with DELETE_INFRA_MACHINE_ON_FAILURE_AFTER after K3S_UPGRADER_UNINSTALL_CONCURRENCY', () => {
      const k3sIndex = PROVISIONING_SETTINGS.indexOf(SETTING.K3S_UPGRADER_UNINSTALL_CONCURRENCY);
      const deleteInfraIndex = PROVISIONING_SETTINGS.indexOf(SETTING.DELETE_INFRA_MACHINE_ON_FAILURE_AFTER);

      expect(deleteInfraIndex).toBeGreaterThan(k3sIndex);
      expect(deleteInfraIndex).toBe(k3sIndex + 1);
    });
  });
});
