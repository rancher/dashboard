...
describe('plugin-products-type-guards', () => {
  ...
  describe('isProductChildWithComponentAndResourceConfig', () => {
    it('should return true when child has component and customResourceConfig', () => {
      const child = {
        ...
        customResourceConfig: {
          ...
          config: { subTypes: ['fleet.cattle.io.gitrepo'] },
        },
      } as any;

      expect(isProductChildWithComponentAndResourceConfig(child)).toBe(true);
    });

    it('should return false when child has component but no customResourceConfig', () => {
      const child = {
        name: 'page', label: 'Page', component: { name: 'Page' }
      } as any;

      expect(isProductChildWithComponentAndResourceConfig(child)).toBe(false);
    });

    it('should return false when child has customResourceConfig but no component', () => {
      const child = {
        name:                 'page',
        label:                'Page',
        customResourceConfig: { type: 'some-type' },
      } as any;

      expect(isProductChildWithComponentAndResourceConfig(child)).toBe(false);
    });

    it('should return false for a group with component and customResourceConfig', () => {
      const child = {
        name:                 'group',
        label:                'Group',
        component:            { name: 'Page' },
    ...
    it('should return false when customResourceConfig is undefined', () => {
      const child = {
        name:                 'page',
        label:                'Page',
        component:            { name: 'Page' },
        customResourceConfig: undefined,
      } as any;
