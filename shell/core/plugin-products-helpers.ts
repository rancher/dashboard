...
class PluginProductsHelpers {
  ...
  private generateVirtualTypeRouteForNewProduct(parentName: string, childName: string | undefined, options: ProductRegistrationRouteGenerationOptions = {}): RouteRecordRawWithParams {
    ...
    if (startRouteWithProduct) {
      name = childName ? `${ parentName }-c-cluster-${ childName }` : `${ parentName }-c-cluster`;
      path = childName ? `${ parentName }/c/:cluster/${ childName }` : `${ parentName }/c/:cluster`;
    } else {
      name = childName ? `c-cluster-${ parentName }-${ childName }` : `c-cluster-${ parentName }`;
      path = childName ? `c/:cluster/${ parentName }/${ childName }` : `c/:cluster/${ parentName }`;
    }
