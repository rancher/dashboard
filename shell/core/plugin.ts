...
import { PluginResourceType } from '@shell/core/plugin-resource-type';
...
export class Plugin implements IPlugin {
  ...
  public resourceTypeConfigs: PluginResourceType[] = [];
  ...
  configureResourceType(type: string, config: ResourceTypeConfig): void {
    this.resourceTypeConfigs.push(new PluginResourceType(type, config));
  }

  resourceNameFor(apiGroup: string, type: string): string {
    return `${ apiGroup }.${ type }`.trim().toLowerCase();
  }
