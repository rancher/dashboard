...
describe('fleet.cattle.io.gitrepo view', () => {
  beforeEach(() => {
    jest.clearAllMocks();
  });
  ...
  describe('view: fleet.cattle.io.gitrepo, GitHub App auth secret creation - should', () => {
    it('create an opaque secret with GitHub App fields', async() => {
      const wrapper = mount(GitRepoComponent, initGitRepo({ mode: _CREATE }));
      const secret = {
        _type:    '',
        data:     null,
        metadata: { name: 'created-github-app-secret' },
        save:     jest.fn().mockResolvedValue(undefined),
      };

      mockStore.dispatch.mockResolvedValue(secret);

      await wrapper.vm.doCreate('clientSecretName', {
        selected:                AUTH_TYPE._GITHUB_APP,
        githubAppId:             '12345',
        githubAppInstallationId: '67890',
        githubAppPrivateKey:     'my-private-key',
      });

      expect(secret._type).toBe(SECRET_TYPES.OPAQUE);
      expect(secret.data).toStrictEqual({
        github_app_id:              base64Encode('12345'),
        github_app_installation_id: base64Encode('67890'),
        github_app_private_key:     base64Encode('my-private-key'),
      });
      expect(secret.save).toHaveBeenCalledWith();
      expect(wrapper.vm.value.spec.clientSecretName).toBe('created-github-app-secret');
    });
  });
});
