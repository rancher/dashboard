<script>
import { set, isEmpty } from '@shell/utils/object';
...
import { SOURCE_TYPE } from '@shell/models/fleet.cattle.io.helmapp.js';
...
export default {
  ...
  data() {
    const sourceTypeOptions = Object.values(SOURCE_TYPE).map((value) => ({
      value,
      label: this.t(`fleet.helmApp.source.types.${ value }`)
    }));
    ...
    return {
      ...
      tempCachedValues:     {},
      valueFromOptions,
      doneRoute:            'c-cluster-fleet-application',
    };
  },
  ...
  computed: {
    ...
    steps() {
      return [
        ...
        {
          name:           'advanced',
          title:          this.t('fleet.helmApp.add.steps.advanced.title'),
          label:          this.t('fleet.helmApp.add.steps.advanced.label'),
          subtext:        this.t('fleet.helmApp.add.steps.advanced.subtext'),
          descriptionKey: 'fleet.helmApp.add.steps.advanced.description',
          ready:          true,
          weight:         1,
        },
      ];
...
<template>
  ...
  >
    ...
    <template #chart>
      <h2 v-t="'fleet.helmApp.source.release.title'" />
      ...
      <div class="row mb-20">
        <div class="col span-6">
          <LabeledInput
            ...
            :mode="mode"
            :label-key="`fleet.helmApp.source.release.label`"
            :placeholder="t(`fleet.helmApp.source.release.placeholder`, null, true)"
            :required="true"
          />
        </div>
      </div>
...
&nbsp;
    ...
    <template #advanced>
      <Banner
        v-if="!isView"
        color="info"
        label-key="fleet.helmApp.add.steps.advanced.info"
      />

      <h2 v-t="'fleet.helmApp.auth.title'" />
      ...
      <SelectOrCreateAuthSecret
        ...
        label-key="fleet.helmApp.auth.helm"
      ...
      <div class="row mt-20 mb-20">
        <div class="col span-6">
          <Checkbox
            ...
            :tooltip="t('fleet.helmApp.tls.insecure')"
            type="checkbox"
            label-key="fleet.helmApp.tls.insecure"
            :mode="mode"
          />
        </div>
      </div>

      <h2 v-t="'fleet.helmApp.resources.label'" />

      <div class="resource-handling">
        <Checkbox
          v-model:value="correctDriftEnabled"
          :tooltip="t('fleet.helmApp.resources.correctDriftTooltip')"
          type="checkbox"
          label-key="fleet.helmApp.resources.correctDrift"
          :mode="mode"
        />
        <Checkbox
          v-model:value="value.spec.keepResources"
          :tooltip="t('fleet.helmApp.resources.keepResourcesTooltip')"
          type="checkbox"
          label-key="fleet.helmApp.resources.keepResources"
          :mode="mode"
        />
      </div>
    </template>
