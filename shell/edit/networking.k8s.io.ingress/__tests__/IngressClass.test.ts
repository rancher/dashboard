...
type IngressClassValue = { spec: { ingressClassName?: string } };
...
describe('ingressClass.vue', () => {
  ...
  it('updates ingressClassName correctly', async() => {
    ...
    expect((wrapper.props('value') as IngressClassValue).spec.ingressClassName).toBe('nginx');
  ...
  it('removes ingressClassName when none is selected', async() => {
    ...
    expect((wrapper.props('value') as IngressClassValue).spec.ingressClassName).toBeUndefined();
    expect(wrapper.emitted()['update:value']).toBeTruthy();
  });
});
