...
export default {
  ...
  data() {
    return {
      mainTabs: [{
          name: DEPLOYMENT_TABS.DEPLOYMENT,
          weight: 99,
        },
        {
          name: DEPLOYMENT_TABS.PODS,
          weight: 98,
      }]
    }
  },
  computed: {
    deploymentTabs() {
      return this.mainTabs.concat(this.containerTabs)
    },

    containerTabs() {
      return this.podTemplateSpec.containers.map((c,i)=>{
        return {
          name: c.name,
          weight: 97 - i, // 99, 98 = DEPLOYMENT_TABS
          label: c.name,
          type: 'container'
        }
      })
  ...
  methods: {
    changed(tab) {
      const containerType = this.containerTabs.find((c) => c.name === tab.tab._props.name && !!tab.tab.__active)
      console.log(containerType)
      if(containerType) {
        this.selectContainer(containerType)
      }
    }
  },
...
<template>
  ...
  <form v-else class="filled-height">
    ...
    >
      ...
      <Tabbed :key="containerChange + 100" @changed="changed">
        <Tab v-for="(tab, i) in deploymentTabs" :key="i" :label="tab.label || t(`workload.container.titles.${tab.name}`)" :name="tab.name" :weight="tab.weight">
          {{tab.type}}
          <Labels v-if="tab.name === 'deployment' " v-model="value" :mode="mode" />
          <div v-if="tab.name === 'pods' " >
