/**
 * Shared flat ESLint config for Rancher UI extensions, exported by `@rancher/shell`.
 *
 * An extension's own `eslint.config.mjs` just re-exports this (and adds any project-specific
 * overrides), so the ruleset lives in one place instead of being copied into every extension:
 *
 *   import shellConfig from '@rancher/shell/eslint.config.base.mjs';
 *   export default [...shellConfig];
 *
 * The eslint toolchain imported below (eslint, neostandard, typescript-eslint, eslint-plugin-vue,
 * vue-eslint-parser, eslint-plugin-cypress, eslint-plugin-import, ...) is provided transitively by
 * `@rancher/shell`.
 *
...
// Flat config forbids one plugin name mapping to two different objects. A fresh extension
// install can end up with two physical copies of `@typescript-eslint/eslint-plugin` (one nested
// under `neostandard`, one from the `typescript-eslint` umbrella), so `neostandard({ts:true})`
// and `tseslint.configs.recommended` each register a *different* instance under the same name.
// Collapse every plugin name to its first-seen instance so all references share one object.
const dedupePlugins = (configs) => {
  const seen = {};

  return configs.map((entry) => {
    if (!entry || !entry.plugins) {
      return entry;
    }
...
const rawExtensionEslintConfig = [
  {
    ignores: [
      '**/*.tsx',
      '**/*.jsx',
...
export const extensionEslintConfig = dedupePlugins(rawExtensionEslintConfig);
