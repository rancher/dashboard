<script>
import FleetHelmApps from '@shell/components/fleet/FleetHelmApps';
...
export default {
  ...
  async fetch() {
    ...
    try {
      const permissions = await checkPermissions({ workspaces: { type: FLEET.WORKSPACE }, helmApps: { type: FLEET.HELM_APP } }, this.$store.getters);
...
<template>
  <div v-if="hasWorkspaces">
    <Masthead
      ...
      :create-button-label="t('fleet.helmApp.actions.add')"
    />
    <FleetHelmApps
      :rows="rows"
      :schema="schema"
