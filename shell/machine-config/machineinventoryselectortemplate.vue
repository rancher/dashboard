...
<template>
  <div>
    <h2
      v-t="'elemental.clusterGroup.selector.label'"
      class="mt-20 mb-20"
    />
    <MatchExpressions
      :mode="mode"
    ...
    />
    <Banner
      v-if="matchingMachineInventories"
      :color="(matchingMachineInventories.isNone || matchingMachineInventories.isAll ? 'warning' : 'success')"
    >
      <span
        v-if="matchingMachineInventories.isAll"
        v-html="t('elemental.clusterGroup.selector.matchesAll', matchingMachineInventories)"
      />
      <span
        v-else-if="matchingMachineInventories.isNone"
        v-html="t('elemental.clusterGroup.selector.matchesNone', matchingMachineInventories)"
      />
      <span
        v-else
