...
export default {
  ...
  methods:  {
    getCountForResource(resourceName) {
      ...
      console.log('individual resource count', resourceName, resourceCount);

      return resourceCount || 0;
    },
    gatherManualRefreshData(dataKey, isDataAnArray, multipleResources = []) {
      ...
      this.isDataAnArray = isDataAnArray;
      ...
      console.log('*** MIXIN ::: isDataAnArray ***', isDataAnArray);
      ...
      if ( this.$store.getters[`${ inStore }/haveAll`](COUNT) ) {
        ...
        if (this.multipleResources.length) {
          this.multipleResources.forEach((type) => {
            resourceCount = resourceCount + this.getCountForResource(type);
          });
        } else {
