...
export default class MgmtCluster extends SteveModel {
  ...
  pin() {
    pinCluster(this.$rootGetters, (action, payload) => this.$dispatch(action, payload, { root: true }), this.id);
  }

  unpin() {
    unpinCluster(this.$rootGetters, (action, payload) => this.$dispatch(action, payload, { root: true }), this.id);
  }
