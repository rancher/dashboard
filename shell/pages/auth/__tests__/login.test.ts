...
/**
 * Returns the key verbatim, as the bulk of the tests assert on keys. `login.logoAlt`
 * is the exception: its whole point is that the vendor name is interpolated into the
 * logo's accessible name, so it has to be rendered to be asserted on.
 */
const t = (key: string, args?: Record<string, string>) => (key === 'login.logoAlt' ? `${ args?.vendor } Logo` : key);
