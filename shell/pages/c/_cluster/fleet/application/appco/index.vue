...
import { APP_STATUS, compatibleVersionsFor, normalizeFilterQuery } from '@shell/store/catalog';
...
import { CATALOG as CATALOG_TYPES, CATALOG_SORT_OPTIONS, MANAGEMENT } from '@shell/config/types';
...
export default {
  ...
  async fetch() {
    ...
    this.installedApps = await this.$store.dispatch('management/findAll', { type: CATALOG_TYPES.APP });
  },
  ...
  methods: {
    ...
    filterCharts({ searchQuery }) {
      ...
      return enabledCharts.filter((c) => {
        ...
        return c.repoName === this.appCoRegistry?.metadata.name;
      });
    },
