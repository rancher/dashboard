...
import { _CREATE, SUB_TYPE } from '@shell/config/query-params';
...
export default {
  ...
  data() {
    return {
      ...
      application: {
        parentNameOverride: this.$store.getters['i18n/t'](`typeLabel."${ FLEET.APPLICATION }"`, { count: 1 })?.trim(),
      },
      abbrSizes:       {
        3: '24px',
        4: '18px',
    ...
    types() {
      return [
        ...
        return {
          ...
          icon:        `icon ${ type === FLEET.GIT_REPO ? 'icon-github' : 'icon-linux' }`,
        };
      });
    },
