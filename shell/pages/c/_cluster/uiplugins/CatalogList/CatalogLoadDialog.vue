  ...
  methods: {
        ...
        if (!isEmpty(this.deploymentValues.spec.template.spec.containers[0].image)) {
          ...
          if (name) {
            ...
            if (this.extensionRepo) {
              // Create uiplugin crd
              await this.loadPlugin(name, this.extensionRepo.spec.url, image);
            }
    ...
    async loadService(name, btnCb) {
      ...
      try {
        ...
        if (this.extensionSvc) {
          this.extensionUrl = `http://${ this.extensionSvc.metadata.name }.${ UI_PLUGIN_NAMESPACE }:8080`;
        } else {
    ...
    parseRepoValues(chartName) {
      ...
      out.spec.url = `http://${ this.extensionSvc.spec.clusterIP }:${ this.extensionSvc.spec.ports[0].port }`;
      ...
      <template>
        <div>
          ...
          <div class="custom mt-10">
            <div class="fields">
              <LabeledInput
                v-model="deploymentValues.spec.template.spec.containers[0].image"
