...
export default {
  ...
  methods: {
    ...
    async uninstall() {
      ...
      if (plugin.uiplugin?.isDeveloper || (plugin.custom && plugin.uiplugin)) {
      ...
      const pluginApps = apps.filter((app) => {
        if (plugin.custom && app.namespace === UI_PLUGIN_NAMESPACE) {
    ...
    async removePluginImageResources(plugin) {
      ...
      if (selector) {
        const hash = await allHash({
          deployment: this.$store.dispatch('management/findMatching', { type: WORKLOAD_TYPES.DEPLOYMENT, selector }),
          service:    this.$store.dispatch('management/findMatching', { type: SERVICE, selector }),
    ...
    >
      ...
      <div class="mt-10 dialog-panel">
        <div class="dialog-info">
          <p>
            {{ plugin.custom ? t('plugins.uninstall.custom') : t('plugins.uninstall.prompt') }}
          </p>
        </div>
