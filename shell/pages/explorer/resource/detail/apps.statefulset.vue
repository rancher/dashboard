...
const { clusterId, id, schema } = useResourceIdentifiers(WORKLOAD_TYPES.STATEFUL_SET);
const statefulSet = await useFetchResourceWithId(WORKLOAD_TYPES.STATEFUL_SET, id);
const titleBarProps = useDefaultTitleBarData(statefulSet);
...
const resourceTabs = await useDefaultWorkloadResourceTabs(statefulSet, {
  cronJobs: true, metrics: hideMetrics, projectMetrics: true
});

const { referredToBy, refersTo } = await useFetchRelatedResourcesTab(statefulSet);
...
const additionalIdentifyingInformation = useDefaultWorkloadIdentifyingInformation(statefulSet);
const { identifyingInformation, labels, annotations } = useDefaultMetadata(statefulSet, additionalIdentifyingInformation);
const { scaleUp, scaleDown } = useScaling(statefulSet.value.name, () => statefulSet.value.scaleUp(), () => statefulSet.value.scaleDown());
</script>
<template>
  <DetailPage>
    <template #top-area>
