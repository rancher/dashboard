...
const state = {
  ...
  resources: {}
  // [resourceType]: {
  //   hashes: { key/value pairs of resource hashes stored under the resource id },
  //   collection: { key/value pairs of whole resources stored under the resource id },
  //   revion: the most recent revision we have for a collection
  //   lastFullSyncTime: timestamp of the last time this resource was fully synced with the api and recieved a new revision
  // }
};
...
const resourceActions = {
  loadSchema: (schemas) => {
    ...
    schemas.forEach((schema) => {
      // cloning each schema because mutating a function prop is a bad practice even though it wouldn't hurt here
      ...
      delete schemaClone._id;
      delete schemaClone._group;

      state.resources.schema.hashes[schemaClone.id] = hashObj(schemaClone);
      state.resources.schema.collection[schemaClone.id] = schemaClone;
    });
  },
  ...
  loadCount: (count) => {
    ...
    state.resources.count.hashes['count'] = hashObj(count[0]); // there's always only 1 index in the array for count since we treat the whole thing as a single object...
    state.resources.count.collection['count'] = count[0]; // there's always only 1 index in the array for count since we treat the whole thing as a single object...
  },
  loadResources: ({ data }, requestHash) => {
    ...
    resourceActions.mutatePayload(resourceType, true, requestHash); // marked for immediate sending
  },
  setupResourceRequest: ({ type, opt, pagination }, requestHash) => {
    ...
    resourceActions.getFullResourceList(type, requestHash);
  },
  getFullResourceList: (resourceType, requestHash) => {
    ...
    axios({ ...clone(state.resources[resourceType].requestOpt), headers: { Accept: 'application/json' } })
      .then((res) => {
        resourceActions.loadResources(res, requestHash);
  ...
  'resource.start': ({ resourceType }) => {
    socketActions.watching(resourceType === 'counts' ? 'count' : resourceType);
  },
  'resource.stop': ({ resourceType }) => {
    socketActions.stopped(resourceType === 'counts' ? 'count' : resourceType);
  },
  ping:             () => {}, // we catch a bunch of pings, nothing to do with them right now.
  mutatePayload:    (resourceType, immediate = false, requestHash) => {
    // ToDo: resourceType === 'metadata.name'? look at `keyFieldForType(type)` in getters
    // import { keyFieldFor } from '@shell/plugins/dashboard-store/normalize.js';
    // ToDo: gonna let the individual tables pass in sortBy or default to id or _id
    ...
    if (state.resources[resourceType]?.pagination?.currentPage || state.resources[resourceType]?.pagination?.pageSize) {
      ...
      for (let i = indexFrom; i <= Math.min(indexTo, (fullResourceList.length - 1)); i++) {
      ...
      if (state.resources[resourceType]?.pagination?.currentPageHash !== pageHash) {
        ...
        resourcePayload.list = [...page];
      }
    } else {
      // we already know we mutated the resource array so we can just send it
      resourcePayload.list = [...fullResourceList];
    }
    resourcePayload.total = fullResourceList.length;

    if (requestHash) {
      workerActions.sendResponse({
        requestHash,
        payload: { [resourceType]: resourcePayload }
      });
    }
...
const workerActions = {
  ...
  waitingForResponse: (req) => {
    const { requestHash, msg } = req;

    resourceActions.setupResourceRequest(msg, requestHash);
  }
};
