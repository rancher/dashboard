...
// ToDo: installing charts kill the app in type-map for some reason
...
state.worker.maintenanceIntervalId = setInterval(() => {
  // Every 1 second we:
  if (state.baseUrl) {
    resourceActions.fullResourceSync();
...
const resourceActions = {
  ...
  // loadResources: ({ resourceType, revision, data: resources = [] }) => {
  loadResources: ({ data }) => {
  ...
  mutatePayload:    (resourceType, immediate = false) => {
    // ToDo: resourceType === 'metadata.name'?
...
const socketActions = {
  ...
  syncWatches: () => {
    ...
    if (state?.socket?.reference?.isConnected()) {
        ...
        .forEach((watch) => {
          ...
          if (watch.status === PENDING) {
            ...
            if (ok) {
              ...
              // ToDo: this should be a resourceAction
      ...
      state.socket.watches = watchesArray
        ...
        // ToDo: clean this mess up... I can use a loop to mutate the original or a copy and I wouldn't have to force it into an array and back into an object...
  ...
  refresh: (type) => {
    const { REFRESHING } = state.socket.watchStatuses;

    state.socket.watches[type].status = REFRESHING;
  },
  watch: (msg) => {
    // ToDo: this can send multiple requests over the socket for the same resource which isn't cool
    const { PENDING } = state.socket.watchStatuses;

    // ToDo: if we're watching a resourced that's already being watched then we should kill the watch before we resubscribe (with the newer revision probably)
  ...
  watching: (type) => {
    const { WATCHING } = state.socket.watchStatuses;

    state.socket.watches[type].status = WATCHING;
  },
  unwatch: (type) => {
    const { CANCELLED } = state.socket.watchStatuses;
