...
describe('prefs store', () => {
  ...
  describe('actions', () => {
    ...
    describe('setMerge', () => {
      ...
      afterEach(async() => {
        // A logged-out setMerge populates module-level prefsBeforeLogin; drain it via loadServer so it
      ...
      it('setMerge chains the optimistic phase then the reconcile phase', async() => {
        const s: any = state();

        s.data[RECENT_CLUSTERS] = ['b'];
        const commit = jest.fn((name: string, payload: any) => {
          if (name === 'load') {
            s.data[payload.key] = payload.value;
          }
        ...
        const server = {
          data: { [RECENT_CLUSTERS]: JSON.stringify(['b']) },
          save: jest.fn().mockResolvedValue(undefined),
        };
        // Route the two sub-actions to their real implementations; anything else (loadServer) → server.
        const dispatch: any = jest.fn((action: string, payload: any) => {
          const ctx = {
            dispatch, commit, rootGetters: loggedIn, state: s
          } as any;

          if (action === 'applyPrefsOptimistic') {
            return Promise.resolve(actions.applyPrefsOptimistic(ctx, payload));
          }
          if (action === 'reconcilePrefs') {
            return actions.reconcilePrefs(ctx, payload);
          }

          return Promise.resolve(server);
        });

        await actions.setMerge({
          dispatch, commit, rootGetters: loggedIn, state: s
        } as any, [{ key: RECENT_CLUSTERS, apply: prepend('a') }]);
