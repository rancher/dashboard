...
export const actions = {
  ...
  // `setMerge` just chains the two for callers that don't need the instant-UI split. SURE-8192.
  ...
  async setMerge(
    { dispatch }: PrefsActionContext,
    mutations: Array<{ key: string, apply: (value: any) => any }>
  ): Promise<PrefError | undefined> {
    const optimistic = await dispatch('applyPrefsOptimistic', mutations);

    return dispatch('reconcilePrefs', { mutations, optimistic });
  },
  ...
  async loadServer( {
    ...
    // `ignoreKey` may be a single key or an array of keys (a batched setMerge write ignores all its keys,
