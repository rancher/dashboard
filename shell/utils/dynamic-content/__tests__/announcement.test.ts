...
describe('processAnnouncements', () => {
  ...
  test('should return early if no announcements are provided', async () => {
    await processAnnouncements(mockContext, undefined, VERSION_270);
    expect(mockDispatch).not.toHaveBeenCalled();
    expect(mockLogger.info).not.toHaveBeenCalled();
  });

  test('should return early if announcements array is empty', async () => {
    await processAnnouncements(mockContext, [], VERSION_270);
    expect(mockDispatch).not.toHaveBeenCalled();
    expect(mockLogger.info).not.toHaveBeenCalled();
  });

  test('should return early if versionInfo is undefined', async () => {
    await processAnnouncements(mockContext, [{ id: '1', target: 'notification/announcement', title: 'Test', message: 'Msg' }], undefined as any);
    expect(mockDispatch).not.toHaveBeenCalled();
    expect(mockLogger.info).not.toHaveBeenCalled();
  });

  test('should return early if versionInfo.version is undefined', async () => {
    await processAnnouncements(mockContext, [{ id: '1', target: 'notification/announcement', title: 'Test', message: 'Msg' }], { version: undefined as any, isPrime: false });
    expect(mockDispatch).not.toHaveBeenCalled();
    expect(mockLogger.info).not.toHaveBeenCalled();
  });

  // --- Version and Audience Filtering ---
  test('should not process announcement if version does not satisfy requirement', async () => {
    (semver.satisfies as jest.Mock).mockReturnValue(false);
    const announcements: Announcement[] = [
      { id: '1', target: 'notification/announcement', title: 'Test', message: 'Msg', version: 'v2.8.0' },
    ];
  ...
  test('should not process admin-only announcement if user is not admin', async () => {
    mockContext.isAdmin = false;
    const announcements: Announcement[] = [
      { id: '1', target: 'notification/announcement', title: 'Test', message: 'Msg', audience: 'admin' },
    ];
  ...
  test('should process admin-only announcement if user is admin', async () => {
  ...
  test('should process all-audience announcement regardless of admin status', async () => {
    mockContext.isAdmin = false; // Test with non-admin
    const announcements: Announcement[] = [
      { id: '1', target: 'notification/announcement', title: 'Test', message: 'Msg', audience: 'all' },
    ];
  ...
  test('should log error for unsupported announcement target type', async () => {
    const announcements: Announcement[] = [
      { id: '1', target: 'unsupported/type', title: 'Test', message: 'Msg' },
    ];
  ...
  test('should log error for unsupported notification sub-type', async () => {
    const announcements: Announcement[] = [
      { id: '1', target: 'notification/unsupported', title: 'Test', message: 'Msg' },
    ];
  ...
  test('should log error and not add notification if announcement has no ID', async () => {
    const announcements: Announcement[] = [
      { target: 'notification/announcement', title: 'Test', message: 'Msg' } as Announcement, // Missing ID
    ];
  ...
  test('should not add notification if one with the same ID already exists', async () => {
    ...
    const announcements: Announcement[] = [
      { id: announcementId, target: 'notification/announcement', title: 'Test', message: 'Msg' },
    ];
  ...
  test('should not add notification if announcement ID is in READ_ANNOUNCEMENTS preference', async () => {
  ...
  test('should add a new announcement notification with default level (announcement)', async () => {
    ...
    const announcements: Announcement[] = [
      { id: announcementId, target: 'notification/announcement', title: 'New Announcement', message: 'This is a new message.' },
    ];
  ...
  test('should add a new info level notification', async () => {
    const announcementId = 'new-info';
    mockGetters['notifications/item'].mockReturnValue(undefined);
    mockGetters['prefs/get'].mockReturnValue('');
    const announcements: Announcement[] = [
      { id: announcementId, target: 'notification/info', title: 'Info Title', message: 'Info Message' },
    ];
  ...
  test('should add a new warning level notification', async () => {
    const announcementId = 'new-warning';
    mockGetters['notifications/item'].mockReturnValue(undefined);
    mockGetters['prefs/get'].mockReturnValue('');
    const announcements: Announcement[] = [
      { id: announcementId, target: 'notification/warning', title: 'Warning Title', message: 'Warning Message' },
    ];
  ...
  test('should add notification with primary CTA', async () => {
    ...
    expect(notification.primaryAction).toEqual({ label: 'Click Me', target: '/some/path' });
    expect(notification.secondaryAction).toBeUndefined();
  });

  test('should add notification with secondary CTA', async () => {
    ...
    expect(mockDispatch).toHaveBeenCalledTimes(1);
    const notification = mockDispatch.mock.calls[0][1];
    expect(notification.secondaryAction).toEqual({ label: 'More Info', target: 'http://example.com' });
    expect(notification.primaryAction).toBeUndefined();
  });

  test('should add notification with both primary and secondary CTAs', async () => {
    ...
    const announcements: Announcement[] = [
      {
        ...
        message: 'Message',
        cta: {
          primary: { action: 'Primary', link: '/primary' },
          secondary: { action: 'Secondary', link: '/secondary' },
        },
      },
    ];
    ...
    expect(mockDispatch).toHaveBeenCalledTimes(1);
    const notification = mockDispatch.mock.calls[0][1];
    expect(notification.primaryAction).toEqual({ label: 'Primary', target: '/primary' });
    expect(notification.secondaryAction).toEqual({ label: 'Secondary', target: '/secondary' });
  });

  // --- Multiple Announcements ---
  test('should process multiple announcements correctly, skipping invalid ones', async () => {
    ...
    const announcements: Announcement[] = [
      { id: 'valid-1', target: 'notification/info', title: 'Valid 1', message: 'Msg 1', audience: 'all' },
      { id: 'existing-id', target: 'notification/announcement', title: 'Existing', message: 'Msg Existing' }, // Should be skipped
      { id: 'read-id', target: 'notification/warning', title: 'Read', message: 'Msg Read' }, // Should be skipped
      { id: 'valid-2', target: 'notification/announcement', title: 'Valid 2', message: 'Msg 2', audience: 'admin' },
      { id: 'invalid-target', target: 'unsupported/type', title: 'Invalid', message: 'Msg Invalid' }, // Should log error
      { id: 'valid-3', target: 'notification/info', title: 'Valid 3', message: 'Msg 3', version: 'v1.0.0' }, // semver.satisfies is mocked to true by default
    ];
