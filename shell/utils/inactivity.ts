...
//     if (isEqual(normalized, [excludeHarvesterRule])) {
//       mode = 'all';
//     }
...
export async function checkIfUIInactivityIsEnabled(store: any): Promise<UIInactivitySettingResponse> {
  ...
  if (settings?.inactivity?.enabled) {
    const data = parseTTLData(settings?.inactivity, 'threshold');

    console.debug(`UI inactivity modal will show after ${ data.showModalAfter || 0 / 60 }(m) and be shown for ${ data.courtesyTimer || 0 / 60 }(m)`); // eslint-disable-line no-console
...
// function checkIfSessionIdleIsEnabled(): Promise<UIInactivitySettingResponse> {}

function parseTTLData(value: any, key: string): UIInactivitySettingResponse {
  // Total amount of time before the user's session is lost
  const thresholdSeconds = value?.[key] * 60;
