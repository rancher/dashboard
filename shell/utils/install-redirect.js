...
export function createInstallRedirectMixin(product, chartName, defaultResourceOrRoute, install = true) {
  return {
    beforeCreate() {
      performRedirect(product, chartName, defaultResourceOrRoute, install, this.$root.$options.context.redirect, this.$store);
    }
  };
}

async function performRedirect(product, chartName, defaultResourceOrRoute, install, redirect, store) {
