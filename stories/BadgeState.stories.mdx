
import { Canvas, Meta, Story, ArgsTable, Source } from '@storybook/addon-docs';
import BadgeState from '../components/BadgeState.vue';
import dedent from 'ts-dedent';

<Meta title="Components/BadgeState" component={BadgeState} />

export const Template = (args, { argTypes }) => ({
  components: { BadgeState },
  props:      Object.keys(argTypes),
  template:   '<BadgeState v-bind="$props" />',
})

<Source
  language='js'
  light
  format={false}
  code={`
     import BadgeState from '../components/BadgeState.vue';
  `}
/>

# BadgeState
Badges are used to highlight important changes of state in an element, mark them, or draw attention to counters.


### How to start

<br/>

### Description

- Use badges to highlight or differentiate key elements. However, do it with caution. Drawing attention to many pieces may result in creating much noise. 
- Badges are good for counters, for example, to show the number of new messages.
- Badges should not provide interaction. However, its parent element may.
- Don't use long texts within a badge. Whenever possible, limit it to a maximum of 3 words or less. 

### States of badge

Different contextual variations of the badge can be used to communicate the different states of an element or the content. Use visually hidden text to convey meaning to assertive technologies.

#### Info
<Canvas>
  <Story
    name="Info"
    args={{
      label: 'Hello world',
      color: 'bg-info'
    }}>
    {Template.bind({})}
  </Story>
  
</Canvas>

#### Warning
Info about warning
<Canvas>
  <Story
    name="Warning"
    args={{
      label: 'Hello World',
      color: 'bg-warning'
    }}>
    {Template.bind({})}
  </Story>
</Canvas>

#### Error

<Canvas>
  <Story
    name="Error"
    args={{
      label: 'Hello World',
      color: 'bg-error'
    }}>
    {Template.bind({})}
  </Story>
</Canvas>

<ArgsTable of={BadgeState} />