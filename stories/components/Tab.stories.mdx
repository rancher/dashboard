...
export const AddRemoveDefault = (args, { argTypes, events }) => {
  const [_, updateArgs] = useArgs();
  return {
    components: { Tabbed, Tab },
  props:      Object.keys(argTypes),
  ...
  template:   `
    <Tabbed v-bind="$props">
      ...
      <div>{{ g.content }}
      <br/>
      <button type="button" class="btn-sm role-link" @click="remove($event)">
        <i class="icon icon-minus icon-lg" /> Remove
      </button>
      </div>
      </Tab>
      <template #tab-row-extras>
        <li class="tablist-controls">
          <button type="button" class="btn-sm role-link" @click="update($event)">
...
- Tabs can either be arranged horizontally or vertically. 
- The current tab defines what content is shown in the tab pane. Tabs can be nested in two levels.
...
### Add/Remove Default Tab
<Canvas>
  <Story
    name="Add/Remove Default Tab"
    args={{
      tabs: [
        {
          "name": "Tab1",
