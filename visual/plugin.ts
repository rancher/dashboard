...
const checkSnapshotExistence = ({
  ...
  const snapshotsDir = customSnapshotsDir ? path.join(process.cwd(), customSnapshotsDir, specFileName) : path.join(
    screenshotsFolder,
    '..',
    'snapshots',
    specRelativeFolder,
...
const runImageDiffAfterScreenshot = async(
  ...
  const diffDir = customDiffDir ? path.join(process.cwd(), customDiffDir, specFileName) : path.join(snapshotsDir, DEFAULT_DIFF_DIR);
  ...
  const {
    pass, added, updated, diffOutputPath
  } = snapshotResult;
